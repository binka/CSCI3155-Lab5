object Lab5 extends jsy.util.JsyApplication {
   ...
   * by '<'... '>'.
   * 
   * Replace 'lisa8891' in the object name above with your IdentiKey.
   * 
  ...
  def typeInfer(env: Map[String,(Mutability,Typ)], e: Expr): Typ = {
    ...
    e match {
      ...
      case Unary(Cast(t1), e1) => 
        if (castOk(typ(e1), t1)) t1 else err(typ(e1), e1)
  ...
  def step(e: Expr): DoWith[Mem, Expr] = {
    ...
    e match {
      ...
      //SearchAssign2
      case Assign(e1, e2) => val e2p_with = step(e2);
